...
class BaiduSpider(BaseSpider):
    ...
    def search_wenku(
        ...
        - 筛选范围：
            BaiduSpider支持五种筛选范围：全部、VIP不限次、VIP、免费、精品。你可以使用`scope`参数来定义此次搜索
            范围。`scope`参数的取值可以是下列任一一项：['all', 'vip-unlimited', 'vip', 'free', 'high-quality']。
            他们的含义与上文所述一致。样例：
            ```python
        ...
        result = self.EMPTY
        # 范围筛选
        _ = ["all", "vip-unlimited", "vip", "free", "high-quality"]
        if _.index(scope) > 0:
            scope = _.index(scope)
        else:
            scope = 0
        ...
        try:
            url = (
                "https://wenku.baidu.com/search?od=0&page=search&sort_type=0&type=sort_click&org=0&word=%s&pn=%d&fd=%d&lm=%d&pt=%d&od=%d"
